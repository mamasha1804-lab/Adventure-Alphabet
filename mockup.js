...
const HQ={
 AF:{dir:'assets/hq40',parts:['AF.00','AF.01','AF.02','AF.03']},
 GM:{dir:'assets/hq40',parts:['GM.00','GM.01','GM.02']},
 TZ:{dir:'assets/hq70',parts:['TZ.00','TZ.01','TZ.02','TZ.03']}
};
const cache={};let renderToken=0;
async function assembledSource(key){
 if(cache[key])return cache[key];
 const spec=HQ[key];
 if(!spec)return cache[key]=`assets/selector-${key}.webp?v=20260821-hq-fallback`;
 try{
  const chunks=await Promise.all(spec.parts.map(p=>fetch(`${spec.dir}/${p}?v=20260821`).then(r=>{if(!r.ok)throw new Error(r.status);return r.text()})));
  const b64=chunks.join('').replace(/\s+/g,'');
  if(b64.length<30000)throw new Error('HQ asset incomplete');
  return cache[key]=`data:image/webp;base64,${b64}`;
 }catch(e){console.warn('HQ selector fallback',key,e);return cache[key]=`assets/selector-${key}.webp?v=20260821-hq-fallback`}
}
