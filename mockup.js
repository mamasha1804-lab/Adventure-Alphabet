...
function assetFile(name){return BASE+'assets/'+encodeURIComponent(name)+'?v=20260824-af-corrected';}
...
function renderExactSelector(){const def=PAGES[page]||PAGES[0];selectorScreen.classList.add('mockup-mode');stage.classList.remove('hidden','is-loading');drawHotspots(def);img.alt=`Учимся с Ларисой Коротаевой — выбор букв ${def.label}`;img.style.opacity='1';img.src=assetFile(def.file);}
...
document.addEventListener('keydown',e=>{if(!letterScreen.classList.contains('hidden'))return;if(e.key==='ArrowLeft'){page=(page+3)%4;renderSelector();}if(e.key==='ArrowRight'){page=(page+1)%4;renderSelector();}});renderSelector();
})();